...
print(f"")
print(f"  DESI BAO DR1 (2024) → DR2 (2025) [arXiv:2503.14738]:")
print(f"    DR2+CMB+DES-SN5YR: w₀ = -0.75 ± 0.07, w_a = -0.86 ± 0.25")
print(f"         ΛCDM excluded at 4.2σ (2.8σ with Pantheon+, 3.8σ with Union3)")
print(f"         Phantom crossing at z ~ 0.5 (w > -1 today, w < -1 in past)")
print(f"         Trajectory follows w_a ≈ -3(1 + w₀)")
print(f"    Bayesian caveat: arXiv:2603.05472 argues 3.1σ (CMB-only)")
print(f"         reduced under Bayesian analysis; not yet 5σ discovery")
...
print(f"    DESI DR2+CMB+DES-SN5YR: w₀ = -0.75 ± 0.07, w_a = -0.86 ± 0.25")
print(f"    Deviation: w₀ at {abs(w0_STUR - (-0.75)) / 0.07:.1f}σ, "
      f"w_a at {abs(wa_STUR - (-0.86)) / 0.25:.1f}σ from DESI DR2")
print()

# JWST early galaxy tension
print(f"  JWST EARLY GALAXY TENSION (spectroscopically confirmed):")
print(f"    MoM-z14: z = 14.44 (current record, McCaffrey+ 2025)")
print(f"    JADES-GS-z14-0: z = 14.18, M★ ~ 5×10⁸ M☉ (290 Myr after BB)")
print(f"      ALMA [OIII] 88μm at 6.67σ (Carniani+ 2024, Nature 624, 85)")
print(f"    JADES-GS-z14-1: z = 13.86, M★ ~ 4×10⁷ M☉ (spectroscopic)")
print(f"    ZF-UDS-7329: >10¹¹ M☉ formed at z ~ 11 (already quiescent)")
print(f"      Baryon-to-DM fraction exceeds cosmic mean")
print(f"    RUBIES-EGS-QG-1: ~10¹¹ M☉ at z = 4.9, all stars in 200 Myr burst")
print(f"")
print(f"    Quantitative tension (Boylan-Kolchin 2023, Nat. Astron. 7, 731):")
print(f"      Required baryon→star efficiency: ε > 0.80 (max expected: 0.30)")
print(f"      At z~9: ε = 0.99 needed (essentially impossible in ΛCDM)")
print(f"      Stellar mass density ~50× ΛCDM max at z~10 (>3σ, Lovell+ 2023)")
print(f"      UV luminosity function barely evolves z=10→14 (should drop steeply)")
print(f"")
print(f"    STUR RESOLUTION: Chronomagnetic modulation provides")
